...
class zenimaxParserBot(Bot):
	...
	reports=ast.literal_eval(reports)
		...
		#Verificar qual e o inicio e o fim do array que contem a informacao do report 
       		while count<len(pre_parsed_lines):
	    	      if pre_parsed_lines[count].startswith('Evidentiary Information'):
	       		 startpos=count+2
            	      if pre_parsed_lines[count].startswith('Bay ID') and startpos!=0 and endpos==0:	
	       		 endpos=count+1
	    	      count+=1
		...
		if startpos!=endpos:
		 ...
		 event.add('additional_information','Content name:'+parsed_line_report['Asset'] + ' | File name:'+parsed_line_report['File Name'] + ' | File size:'+parsed_line_report['File Size'])
		 date_value=datetime.strptime(parsed_line_report['Last Seen Date'][:-4],'%d %b %Y %H:%M:%S').strftime('%Y-%m-%d %H:%M:%S') + " UTC"
		 event.add('source_time',date_value)
                 event.add('feed', 'RT-Zenimax Media')
                 event.add('feed_code', 'Zenimax Media')
...
if __name__ == "__main__":
    bot = zenimaxParserBot(sys.argv[1])
    bot.start()
