	...
	reports=ast.literal_eval(reports)
		...
		#Verificar qual e o inicio e o fim do array que contem a informacao do report 
       		while count<len(pre_parsed_lines):
	    	      if pre_parsed_lines[count].startswith('Evidentiary Information'):
	       		 startpos=count+2
            	      if pre_parsed_lines[count]=='' and startpos!=0 and endpos==0 and count!=startpos-1:	
	       		 endpos=count
	    	      count+=1
		...
		if startpos!=endpos:
		 ...
		 if badparsing==0:
		  ...
		  date_value=datetime.strptime(parsed_line_report['Initial Infringement Timestamp'][:-4],'%d %b %Y %H:%M:%S').strftime('%Y-%m-%d %H:%M:%S') + " UTC"
		  event.add('source_time',date_value)
                  event.add('feed', 'RT-BroadGreen')
                  event.add('feed_code', 'BroadGreen')
...
if __name__ == "__main__":
    bot = broadgreenParserBot(sys.argv[1])
    bot.start()
