...
class GEANTRDPDICTParserBot(Bot):
		...
		if startpos+1!=endpos:
		 ...
		 for eventline in eventlist:
		    if (eventline['Flags'].find('....S.'))!=-1:
		     ...
		     event.add('application_protocol','rdp')
		     event.add('type', 'brute-force')
		     event.add("additional_information","RDP brute-force attempt occurred")
...
if __name__ == "__main__":
    bot = GEANTRDPDICTParserBot(sys.argv[1])
    bot.start()
